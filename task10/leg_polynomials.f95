...
function make_legendre (n) result (p)
    implicit none
    integer,intent(in) :: n
    real(mp), dimension (0:n) :: p
    integer :: i
    real(mp), dimension (0:n) :: p1, p2 ! p1=p_(n-1), p2=p_(n-2)
    ...
    select case (n)
        ...
        case default
            p2(n) = 1.0_mp ! p_0(x) = 1
            p1(n-1) = 1.0_mp ! p_1(x) = x

            do i = 2,n
                p(:n-1) = p1(1:)
                p(n) = 0.0_mp
                p =((2.0_mp*real(i)- 1.0_mp) * p - (real(i)-1.0_mp) * p2) / real(i)
                p2 = p1
                p1 = p
            end do
    end select
...
function leg_roots(p) result(x)
    real(mp), intent(in) :: p(0:)
    real(mp) :: x(1:size(p)-1), a1(0:size(p)-1)
    integer :: n, k
    n = size(p) - 1
